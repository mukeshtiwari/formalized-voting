...
main = do
  ...
  P.flip forM_ (\x -> do
   ...
   r <- readFile x
   let votes = final_count candEq P.. P.map balfun P.. P.map (P.map (\(x, y) -> (charCand x, y))) P.. P.map L.sort P.. P.map (\x -> P.zip x [1..]) P.. P.lines P.$ r
   P.print votes
   end <- getCurrentTime
