...
type Loses_type = SigT Z (SigT Cand (Prod PathT (SigT ((Prod Cand Cand) -> Bool) ())))

m :: (Cand -> Cand -> Z) -> Nat -> Cand -> Cand -> Z
m edge n c d =
  case n of {
   O -> edge c d;
   S n' -> max (m edge n' c d) (maxlist (map (\x -> min (edge c x) (m edge n' x d)) cand_all))}
...
l10 :: (Cand -> Cand -> Z) -> (Cand -> Cand -> Sumbool) -> Nat -> Z -> Cand -> Cand -> PathT
l10 edge dec_cand n s c d =
  nat_rect (\_ c0 d0 _ -> UnitT c0 d0) (\n0 iHn s0 c0 d0 _ ->
    let {
     c1 = compare0 (m edge n0 c0 d0)
            (maxlist (map (\x -> min (edge c0 x) (m edge n0 x d0)) cand_all))}
    in
    case c1 of {
     Lt ->
      let {h = l9 cand_all dec_cand s0 (\x -> min (edge c0 x) (m edge n0 x d0))} in
      case h of {
       ExistT x _ -> let {iHn0 = iHn s0 x d0 __} in ConsT c0 x d0 iHn0};
     _ -> iHn s0 c0 d0 __}) n s c d __

l15 :: (Cand -> Cand -> Z) -> (Cand -> Cand -> Sumbool) -> Cand -> Cand -> SigT Z
...
l15 edge dec_cand c d =
  ...
  (let {h = l10 edge dec_cand (length cand_all) s c d} in
   Pair h (ExistT (\x -> leb (m edge (length cand_all) (fst x) (snd x)) s) __))
...
l18 :: (Cand -> Cand -> Z) -> (Cand -> Cand -> Sumbool) -> Cand -> SigT Z
...
l18 edge dec_cand c =
  ...
  (l10 edge dec_cand (length cand_all) (add0 s (Zpos XH)) x c) (ExistT (\x0 ->
  ltb (m edge (length cand_all) (fst x0) (snd x0)) (add0 s (Zpos XH))) __)))

wins_loses_M :: (Cand -> Cand -> Z) -> (Cand -> Cand -> Sumbool) -> Cand -> Sum Wins_type Loses_type
wins_loses_M edge dec_cand c =
  let {h = l7 edge c} in
  case h of {
   Left -> Inl (l15 edge dec_cand c);
   Right -> Inr (l18 edge dec_cand c)}
...
data Count =
 ...
 | Fin (Cand -> Cand -> Z) (List Ballot) (Cand -> Bool) (Cand -> Sum Wins_type Loses_type) Count
...
final_count :: (Cand -> Cand -> Sumbool) -> (List Ballot) -> SigT (Cand -> Bool) (SigT Count ())
   ...
   ExistT bs' s ->
    case s of {
     ExistT m0 x0 ->
      ...
      let {x2 = x1 __ __} in ExistT (c_wins m0) (ExistT x2 __)}}
