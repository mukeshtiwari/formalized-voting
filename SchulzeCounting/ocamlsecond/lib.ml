  ...
  | S n' ->
    ...
    (match cm with
     | Lt ->
       ...
       in
       ConsT (c, x, d,
       (iterated_marg_patht cand_all0 dec_cand marg n' s x d))
...
let c_wins cand_all0 marg c =
  forallb (fun d ->
    Z.leb (m cand_all0 marg (length cand_all0) d c)
      (m cand_all0 marg (length cand_all0) c d)) cand_all0
...
let iterated_marg_wins_type cand_all0 dec_cand marg c d =
  ...
  (let hi =
   ...
   (let r = m cand_all0 marg (length cand_all0) d c in
    ExistT ((fun x ->
    Z.leb (m cand_all0 marg (length cand_all0) (fst x) (snd x)) r), __)))))
